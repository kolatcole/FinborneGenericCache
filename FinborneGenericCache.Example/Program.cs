...
public class CacheExample
{
    public static async Task Main(string[] args)
    {
        ...
        await cache.AddAsync("2", "two");      // head is 2, Tail is 3   1 -> 2 -> 3
        await cache.AddAsync("4", "four");     // head is 4, Tail is 3   4 -> 1 -> 2 -> 3
        var three = await cache.GetAsync("3"); // head is 3, Tail is 1   3 -> 4 -> 1 -> 2
        await cache.AddAsync("5", "five");     // head is 5, Tail is 2   5 -> 3 -> 4 -> 1 
